...
const (
	...
	hubPinVerifierStepName     = "Verify production Hub pin"
	goReleaserStepName         = "Run GoReleaser"
	hubPinEnv                  = "QURL_CONNECTOR_HUB_SERVER_PUBLIC_KEY_B64"
	hubPinRepositoryVariable   = "${{ vars.QURL_CONNECTOR_HUB_SERVER_PUBLIC_KEY_B64 }}"
	hubPinVerifierCommand      = "go run ./apps/cli/internal/connector/hub/cmd/verify-production-pin apps/cli/internal/connector/hub/production-public-key.sha256"
	cliWorkflow                = "cli.yml"
...
// native Connector trust chain. Runtime pins the Hub key rather than trusting
...
func TestCLIReleaseVerifiesAndInjectsTheSameHubPin(t *testing.T) {
	t.Parallel()
	...
	for _, path := range []string{
		filepath.Join("..", "..", "apps", "cli", "internal", "connector", "hub", "cmd", "verify-production-pin", "main.go"),
		filepath.Join("..", "..", "apps", "cli", "internal", "connector", "hub", "production-public-key.sha256"),
	} {
