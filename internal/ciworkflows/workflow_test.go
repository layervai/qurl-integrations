...
func TestDiscordDependencyAuditContract(t *testing.T) {
	...
	if install == nil {
		t.Fatal("Discord workflow is missing the Install dependencies step")
	}
	installFields := strings.Fields(install.Run)
	if len(installFields) < 2 || installFields[0] != "npm" || installFields[1] != "ci" {
		t.Errorf("Discord workflow install command = %q, want npm ci", install.Run)
	}
	if !slices.Contains(installFields, "--no-audit") {
		t.Errorf("Discord workflow install still runs npm's implicit audit: %q", install.Run)
	}

	const explicitAudit = "node scripts/audit-production-dependencies.js"
	if audit == nil {
		t.Fatal("Discord workflow is missing the Audit dependencies step")
	}
	if strings.TrimSpace(audit.Run) != explicitAudit {
		t.Errorf("Discord workflow explicit audit = %q, want %q", audit.Run, explicitAudit)
	}
	if timeout, ok := audit.TimeoutMinutes.(int); !ok || timeout != 3 {
		t.Errorf("Discord workflow explicit audit timeout = %#v, want 3 minutes", audit.TimeoutMinutes)
	}

	explicitAuditCommand := regexp.MustCompile(`(?m)^[ \t]*(?:npm[ \t]+audit(?:[ \t]|$)|node[ \t]+scripts/audit-production-dependencies\.js[ \t]*$)`)
	auditGateCount := 0
	for _, job := range workflow.Jobs {
		for index := range job.Steps {
			auditGateCount += len(explicitAuditCommand.FindAllString(job.Steps[index].Run, -1))
		}
	}
	if auditGateCount != 1 {
	...
	logicalDockerfile := regexp.MustCompile(`\\\r?\n[ \t]*`).ReplaceAllString(string(dockerfile), " ")
	runInstructions := regexp.MustCompile(`(?m)^RUN[^\r\n]*`).FindAllString(logicalDockerfile, -1)
	npmCICommand := regexp.MustCompile(`(?:^RUN[ \t]+|(?:&&|\|\||;)[ \t]*)npm[ \t]+ci\b[^&|;\r\n]*`)
	dockerInstalls := make([]string, 0, 1)
	for _, instruction := range runInstructions {
		dockerInstalls = append(dockerInstalls, npmCICommand.FindAllString(instruction, -1)...)
	}
