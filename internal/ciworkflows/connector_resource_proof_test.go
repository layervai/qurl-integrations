...
func TestConnectorResourceProofWorkflowKeepsItsSafetyGates(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		`QURL_CONNECTOR_HUB_HOST must be the exact sandbox NHP Hub name`,
		`hub.nhp.layerv.xyz, which aliases the UDP/443 Hub NLB`,
		`hub.sandbox.nhp.layerv.xyz wildcard resolves to a different, TCP-only`,
		`QURL_CONNECTOR_HUB_HOST: ${{ vars.QURL_CONNECTOR_HUB_HOST }}`,
		`DNS aliases and wildcard names`,
		`HTTP_PROXY: ""`,
		`HTTPS_PROXY: ""`,
		`ALL_PROXY: ""`,
		`NO_PROXY: "*"`,
