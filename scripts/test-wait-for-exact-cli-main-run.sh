...
if [[ "$*" == *"/attempts/"*"/jobs"* ]]; then
  ...
  case "$SCENARIO" in
    jobs_missing)
      jq -n '{total_count:1,jobs:[{name:"cli / exact packaged customer journey",status:"completed",conclusion:"success"}]}'
      ;;
    jobs_skipped)
      jq -n '{total_count:2,jobs:[
        {name:"cli / exact packaged customer journey",status:"completed",conclusion:"skipped"},
        {name:"cli / required",status:"completed",conclusion:"success"}
      ]}'
      ;;
    jobs_required_failed)
      jq -n '{total_count:2,jobs:[
        {name:"cli / exact packaged customer journey",status:"completed",conclusion:"success"},
...
case "$SCENARIO" in
  success|jobs_*) status=completed; conclusion=success ;;
...
run_case success 0
[[ $(cat "$work/run-calls") == 1 && $(cat "$work/job-calls") == 1 ]]
run_case failure 1
