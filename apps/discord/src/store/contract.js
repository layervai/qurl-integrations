...
// `getStats` with a wrong arity / wrong return shape passes
...
/**
 * Verifies a Store backend implements every method the bot code
 * depends on. Called once at boot (see
...
function assertStoreShape(store, backend) {
  ...
  throw new Error(`Store backend '${backend}' is incomplete — missing methods: ${missingMethods.join(', ')}. Check that the backend module exports every item listed in src/store/contract.js's STORE_METHODS.`);
}

module.exports = { STORE_METHODS, assertStoreShape };
