...
function buildAuth0AuthorizeUrl({ state, codeChallenge }) {
  ...
  // `login` invalidates the ambient Auth0 session. Until #1365 pins the
  // passwordless connection, a live upstream social/enterprise IdP session
  // can still select an account. `consent` lets a setup re-run mint a new key
  // instead of silently reusing an earlier grant.
