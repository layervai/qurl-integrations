...
const TOKENS = 10; // MINT_BATCH_SIZE, asserted against the export below.
...
describe('runRound accounting — request bound', () => {
  it('agrees with the constant the batch plan is built from', () => {
    expect(loadWithCount(10).MINT_BATCH_SIZE).toBe(TOKENS);
  });
});

describe('runRound accounting — all uploads succeed', () => {
  it('uploads once and mints every bounded batch', async () => {
  ...
  it('mints every batch against the same uploaded resource', async () => {
    const { runRound } = loadWithCount(30);
    mockReUploadBuffer
      .mockResolvedValueOnce(upload('res-1'))
      .mockResolvedValueOnce(upload('res-2'))
...
describe('runRound accounting — mint failures', () => {
