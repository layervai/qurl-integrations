...
describe('loadtest script — static checks on call sites no test can reach', () => {
  ...
  it('scans argv through the shared reader and nothing else', () => {
    ...
    // Four readers: the numeric resolver, --file, --ledger and --reclaim. The
    // last two arrived with the reclaim ledger and route through readFlag
    // rather than carrying their own scan, which is what this test is for —
    // the count rises as flags adopt the shared reader, and the bans above are
    // what stay at zero. Raising it is only correct alongside those staying 0.
    expect(callsNamed('readFlag')).toHaveLength(4);
  });
