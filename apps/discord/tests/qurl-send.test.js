...
describe('handleAddRecipients', () => {
  ...
  it('file send: mints new links via connector and delivers DMs', async () => {
    ...
    const result = await handleAddRecipients('send-file-1', 'sender-1', users, mockOriginalInteraction);

    // mintLinks should have been called with the connector resource ID
    expect(mockMintLinks).toHaveBeenCalledWith('conn-res-42', expect.any(String), 2);
  ...
  it('filters bots from mixed collection and sends only to real users', async () => {
    ...
    const result = await handleAddRecipients('send-mixed', 'sender-1', users, mockOriginalInteraction);
  ...
  it('file send: rejects > 10 new recipients (quota cap)', async () => {
    mockDb.getSendConfig.mockReturnValue({
      resource_type: 'file',
    ...
    // 12 recipients exceeds the TOKENS_PER_RESOURCE cap
    const userList = [];
    for (let i = 0; i < 12; i++) {
    ...
    expect(mockMintLinks).not.toHaveBeenCalled();
    expect(result.msg).toMatch(/Cannot add more than 10/);
  });
