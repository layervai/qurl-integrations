...
describe('valuedBooleanFlags — a boolean written with a value is not absence', () => {
  ...
  it('covers every flag the boolean resolver reads', () => {
    ...
    traverse(parser.parse(source, { sourceType: 'script' }), {
      FunctionDeclaration(fnPath) {
        if (fnPath.node.id && fnPath.node.id.name !== 'resolveBooleanArgs') return;
        if (!fnPath.node.id) return;
        // Scoped to resolveBooleanArgs and keyed on its local `read` helper.
        // This used to scan `hasFlag` call sites, but #1176 removed the last
        // one — leaving this traversal matching nothing and asserting zero
        // names, which is how it came to fail on main. readBooleanFlag itself
        // cannot be the target: it takes the flag as a variable, so the only
        // place the names appear as literals is the `read(...)` calls.
        fnPath.traverse({
          CallExpression({ node }) {
            if (node.callee.type !== 'Identifier' || node.callee.name !== 'read') return;
    ...
    // A subset rather than an equality: pinning equality would fail the moment
    // a value-less flag is added that some other reader owns, which is a
    // design choice this check has no business forcing.
