# qURL Gmail Edge Extension Local Unpacked Test Guide

This document explains how to build the extension with `scripts/package-all.sh`, load the unpacked `release/` directory into Microsoft Edge, and verify the end-to-end upload flow in Gmail without using the ZIP package.

## Scope

- Build the extension locally
- Load the unpacked `release/` directory in Edge
...
- The project root at `apps/edge-extension/`
...
- `release/` contains the unpacked extension files used by Edge
- `dist/` contains a ZIP package for Microsoft Edge Add-ons upload, which is not needed for this local test
...
> **Note:** This guide's screenshots (steps 4–9) were captured in Chrome, before the qURL
> rebrand — their popup header reads `QURL File Upload`, whereas this extension's `ext_name`
> is now **qURL File Upload for Edge**. The flow each one illustrates is identical in Edge.
> Step 2 has no screenshot until a real `edge://extensions` capture — showing the extension
> actually loaded — replaces the Chrome one that used to sit here.
...
Click the extension icon in the Edge toolbar to open the popup.
...
- If a custom qURL server is used, Edge may request host permission when the server is saved
...
2. Edge loads `release/` through **Load unpacked**
