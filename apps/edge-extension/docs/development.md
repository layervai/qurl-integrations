...
The ZIP places `manifest.json` at the root, as Microsoft Edge Add-ons requires.

```text
dist/qurl-gmail-edge-extension-v1.0.0.zip
```

Upload the ZIP from `dist/` to Microsoft Edge Add-ons — not the repository
itself.
...
`package.json`. The seed in `.release-please-manifest.json` is **`1.0.0`** —
intentionally not the `0.1.0` other apps use — because the migrated Edge
package starts from the pre-monorepo Edge release baseline. The
`bump-version.js` scripts above remain a convenience for ad-hoc local Add-ons
ZIPs outside the release flow.
...
## Microsoft Edge Add-ons review

[edge-add-ons-review.md](./edge-add-ons-review.md) explains the
...
[edge-add-ons-submission-guide.md](./edge-add-ons-submission-guide.md) covers
the package metadata, reviewer note, privacy copy, and pre-submission checks
