...
The committed PNGs are CI-verified byte-identical to a fresh `npm run icons` (`test/generate-icons.test.js`), which is why a `sharp` upgrade has to regenerate and commit them in the same PR: the encoder, not just the logo, determines the bytes. This drift went unnoticed once already in #908; the standing cost of the byte-exact check is tracked in #1046.
