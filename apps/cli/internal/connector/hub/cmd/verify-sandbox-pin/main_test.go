...
func TestRunAcceptsMatchingSandboxPin(t *testing.T) {
	key, fingerprint := releaseKey(t)
	var stdout, stderr bytes.Buffer
	exitCode := run(
		[]string{"sandbox.sha256"},
		func(name string) (string, bool) {
			if name != hub.ReleaseEnvSandboxServerPublicKey {
				t.Fatalf("lookup env %q", name)
			}
			return key, true
		},
		func(path string) ([]byte, error) {
			if path != "sandbox.sha256" {
	...
	if got, want := stdout.String(), "verified sandbox Hub public-key fingerprint "+fingerprint+"\n"; got != want {
...
func TestRunFailsClosed(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "fingerprint read error", args: []string{"pin"}, candidate: key, candidateOK: true, readErr: errors.New("not found"), wantExit: 1, wantErr: "read sandbox Hub fingerprint: not found", secret: key},
