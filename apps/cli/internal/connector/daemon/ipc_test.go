...
func TestIPCServerReadinessReloadAndShutdown(t *testing.T) {
	...
	// The default daemon accepts runtime headers over its verified tunnel.
	if _, err := client.SetOverlay(context.Background(), map[string]map[string]string{"a": {"X-Origin-Token": "private-value"}}); err != nil {
		t.Fatalf("default daemon refused runtime headers: %v", err)
	}
	if storedOverlay(manager)["a"]["X-Origin-Token"] != "private-value" {
		t.Fatal("daemon did not retain runtime headers")
	}
	if _, err := client.SetOverlay(context.Background(), nil); err != nil {
		t.Fatalf("empty overlay refused: %v", err)
	}
	cancel()
