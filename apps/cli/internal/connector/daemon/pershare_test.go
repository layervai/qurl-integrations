...
func TestPerShareManagerSpendsOneAdmissionAndOneSessionPerShare(t *testing.T) {
	...
	go func() {
		done <- (&IPCServer{SocketPath: socket, Manager: manager, JobVersion: "5/test/per-share"}).Run(ctx)
	}()
	t.Cleanup(func() {
		cancel()
		select {
	...
	if status.JobVersion != "5/test/per-share" || len(status.Running) != 3 || len(status.Resources) != 3 {
