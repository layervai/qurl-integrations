...
func staticTrustConfig(issuerKeys, relays []string) (qurl.Config, error) {
	...
	// so a padded value like " relay.host " is normalized and still matches — passing
	// the raw slice here is safe and no CLI-side trimming is needed. We reject a fully
	// empty/whitespace entry explicitly only to surface a clear CLI error instead of
	// the allowlist silently dropping it (which would mask a typo'd flag).
