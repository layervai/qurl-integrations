...
func TestReleaseBuildInjectsVerifiedSandboxHubPinAndSnapshotsDefaultDark(t *testing.T) {
	...
	for _, want := range []string{
		...
		`-X github.com/layervai/qurl-integrations/apps/cli/internal/connector/hub.defaultSandboxServerPublicKeyB64={{ envOrDefault "QURL_CONNECTOR_SANDBOX_HUB_SERVER_PUBLIC_KEY_B64" "" }}`,
	} {
		if !strings.Contains(ldflags, want) {
			t.Fatalf("qurl build ldflags = %q, missing %q", ldflags, want)
		}
	}
	if strings.Contains(ldflags, ".Env.QURL_CONNECTOR_SANDBOX_HUB_SERVER_PUBLIC_KEY_B64") {
