...
export class TeamsBot {
  ...
  async handleActivity(activity: TeamsActivity, signal?: AbortSignal, reply?: (text: string) => Promise<void>): Promise<void> {
    if ((activity.type ?? '').toLowerCase() === 'conversationupdate') { await this.captureConversation(activity); return; }
    ...
    if (activity.conversation?.conversationType === 'personal') await this.captureConversation(activity);
    let response: string;
    try {
  ...
  async execute(activity: TeamsActivity, tenantId: string, scopeId: string, channel: boolean, command: TeamsCommand, signal?: AbortSignal): Promise<string> {
    ...
    if (['admins', 'add', 'remove', 'uninstall', 'protect-url', 'protect-connector', 'set-alias', 'unset-alias', 'set-display-name', 'unset-display-name', 'revoke'].includes(command.verb) && !admin.isAdmin) throw new Error('This command is limited to the tenant owner and qURL admins.');
    ...
    if (command.verb === 'add' || command.verb === 'remove') {
      if (!mentionedAadObjectId) throw new Error('The Teams user mention has no AAD object id');
    ...
    if (command.verb === 'uninstall') {
      ...
      if (credential && !credential.keyId) throw new Error('Tenant credential has no revocation key id');
      if (credential?.keyId) {
    ...
    if (!channel) throw new Error('This command must be run in a Teams channel.');
    ...
    if (command.verb === 'unset-alias') { await this.#options.data.unbindScopeAlias(tenantId, scopeId, command.resource ?? ''); return `Removed alias \`$${command.resource}\` from this channel.`; }
    ...
    throw new Error(`unsupported command: ${command.verb}`);
  }
  ...
  async protectConnector(qurl: QurlClient, activity: TeamsActivity, tenantId: string, scopeId: string, command: TeamsCommand, signal?: AbortSignal): Promise<string> { const slug = command.resource ?? command.args[0] ?? ''; validateTunnelSlug(slug); const ref = await this.#options.data.personalConversationRef(tenantId, activity.from?.aadObjectId?.trim().toLowerCase() ?? ''); if (!ref) throw new Error('Open a personal chat with the bot before protecting a connector.'); const resources = await this.resources(qurl, signal); const resource = resources.find(item => item.type === 'tunnel' && item.slug === slug) ?? await qurl.createResource({ type: 'tunnel', slug, findOrCreate: true, idempotencyKey: idempotencyKey(tenantId, scopeId, activity.from?.id ?? '', slug, activity.id ?? '') }, signal); await this.#options.data.exposeResource(tenantId, scopeId, resource.resourceId); const alias = command.flags.alias ?? slug; await this.#options.data.bindScopeAlias(tenantId, scopeId, alias, resource.resourceId); const token = await qurl.createEnrollmentToken(slug, idempotencyKey(tenantId, scopeId, activity.from?.id ?? '', slug, activity.id ?? ''), signal); const installText = renderTunnelInstallMessage({ slug, alias, environment: normalizeTunnelEnvironment(command.flags.env ?? 'docker'), port: Number(command.flags.port ?? '8080'), image: this.#options.connectorImage ?? '', bootstrapKey: token.apiKey }); try { await this.#options.messages.sendText(ref.serviceUrl, ref.conversationId, `Connector \`${slug}\` bootstrap instructions:\n${installText}`, signal); } catch (error) { try { await qurl.revokeApiKey(token.keyId, signal); } catch { /* preserve the delivery failure without leaking the bootstrap key */ } throw error; } return `Protected connector \`$${resource.resourceId}\` and sent the bootstrap instructions to your personal Teams chat.`; }
}
