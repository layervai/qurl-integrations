...
export function normalizeTunnelEnvironment(environment: string): TunnelEnvironment { const value = environment.trim().toLowerCase(); if (value === '' || value === 'docker') return 'docker'; if (value === 'compose' || value === 'docker-compose') return 'compose'; if (value === 'ecs-fargate') return 'ecs-fargate'; if (value === 'kubernetes') return 'kubernetes'; throw new UserFacingError('invalid connector environment'); }
