...
export function validateTunnelImageRef(image: string): void { if (image && !/^[A-Za-z0-9./:@_-]+$/.test(image)) throw new Error('invalid connector image reference'); }
export function normalizeTunnelEnvironment(environment: string): TunnelEnvironment { const value = environment.trim().toLowerCase(); if (value === '' || value === 'docker') return 'docker'; if (value === 'compose' || value === 'docker-compose') return 'compose'; if (value === 'ecs-fargate') return 'ecs-fargate'; if (value === 'kubernetes') return 'kubernetes'; throw new Error('invalid connector environment'); }
export function validateTunnelSlug(slug: string): void { if (!slugPattern.test(slug)) throw new Error('connector id must be 3-64 lowercase letters, numbers, or hyphens'); }
export function validateTunnelService(service: string): void { if (service && !servicePattern.test(service)) throw new Error('connector service is invalid'); }

function shellQuote(value: string): string { return `'${value.replaceAll("'", `'"'"'`)}'`; }
export function renderTunnelInstallMessage(args: TunnelInstallArgs): string {
  validateTunnelSlug(args.slug); if (!aliasPattern.test(args.alias)) throw new Error('connector alias is invalid'); if (!Number.isInteger(args.port) || args.port < 1 || args.port > 65_535) throw new Error('connector port is invalid'); validateTunnelImageRef(args.image); validateTunnelService(args.service ?? '');
