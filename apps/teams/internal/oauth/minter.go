...
const (
	...
	// already bound the tenant identity and the Teams bot should show the
	...
	// launch signal that allows Teams setup to fall back to the legacy API-key
...
// the Teams tenant already has an external identity binding, but the bot
...
// MintWorkspaceAPIKey creates the Teams tenant external identity binding
...
// qurl-service also assigns provider scopes server-side; Teams bindings are
...
func (m *HTTPAPIKeyMinter) MintWorkspaceAPIKey(ctx context.Context, accessToken, tenantID string) (WorkspaceAPIKeyMint, error) {
	tenantID = strings.TrimSpace(tenantID)
	if tenantID == "" {
		return WorkspaceAPIKeyMint{}, errors.New("MintWorkspaceAPIKey: empty tenantID")
	}
	displayName := "Teams tenant " + tenantID
	idempotencyKey := bindingIdempotencyKey(tenantID)
...
// Teams stores it.
func (m *HTTPAPIKeyMinter) MintWorkspaceReplacementAPIKey(ctx context.Context, accessToken, tenantID, oldKeyID string) (WorkspaceAPIKeyMint, error) {
	tenantID = strings.TrimSpace(tenantID)
	if tenantID == "" {
		return WorkspaceAPIKeyMint{}, errors.New("MintWorkspaceReplacementAPIKey: empty tenantID")
	}
	oldKeyID = strings.TrimSpace(oldKeyID)
	if oldKeyID == "" {
		return WorkspaceAPIKeyMint{}, errors.New("MintWorkspaceReplacementAPIKey: empty oldKeyID")
	}
	return m.mintLegacyAPIKey(ctx, accessToken, "Teams tenant "+tenantID, apiKeyScopes(), replacementIdempotencyKey(tenantID, oldKeyID))
}
...
func bindingIdempotencyKey(tenantID string) string {
	// qurl-service requires a 32+ character idempotency key. Teams tenant IDs
	// are shorter, so hash to a stable fixed-width key with a readable prefix.
	sum := sha256.Sum256([]byte(tenantID))
	return "teams-tenant-binding-v1-" + hex.EncodeToString(sum[:])
}

func replacementIdempotencyKey(tenantID, oldKeyID string) string {
	// Stable across --rotate retries while DDB still stores oldKeyID. If a
	// replacement is minted but Teams fails to persist it, callers must keep
	...
	// Teams/qURL identifiers; length-prefixing keeps the unhashed value
	...
	return fmt.Sprintf("teams-tenant-rotate-replacement-v1-t%d-%s-k%d-%s", len(tenantID), tenantID, len(oldKeyID), oldKeyID)
}
