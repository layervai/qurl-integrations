...
// base64url-decoded to read the Teams tenant/user IDs and optional setup
// email. Keep secrets out of state; the current fields are bot-verified
// identifiers and the requester-typed email used only for Auth0 account
// selection.
//
...
// setup entrypoint, which has already verified the bot request and therefore
// the caller's tenant identity.
//
// Expiry: 5 minutes from mint covers the bot reply → click → Auth0
// authenticate → callback round-trip.
//
...
// setup-command surface gates both explicit modes identically: they require an
...
func mintState(secret []byte, teamID, userID, email string, mode SetupMode, now time.Time) (string, error) {
	...
	// Current Teams tenant/user IDs do not contain '|', but if that ever
	// changes a stray separator would split into more parts than
...
// MintStateWithEmail produces a fresh state token binding the Teams tenant/user
