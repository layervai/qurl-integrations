...
// os.Args — two of G705's configured sources. So every value in its return tuple arrives
// tainted; a returned compile-time constant would too. The write is safe on its own
// terms anyway: stderr on a CLI is not a browser, and slacksmoke.IsEnvVarName has
