...
func TestLogAgentSurfaceState_ConfirmMode(t *testing.T) {
	...
	wired := agentSurfaceState{llmWired: true, storeWired: true, postWired: true, blocksWired: true, dmBlocksWired: true, ephemeralBlocksWired: true}
	cases := []struct {
		name        string
		state       agentSurfaceState
		...
		// CONFIRM live but a get-delivery block seam unwired → the loud post-mint-failure warning.
		{name: "confirm live but dm-blocks unwired", state: with(wired, func(s *agentSurfaceState) { s.confirmFlag = true; s.dmBlocksWired = false }), wantConfirm: "get approvals will FAIL after minting"},
		{name: "confirm live but ephemeral-blocks unwired", state: with(wired, func(s *agentSurfaceState) { s.confirmFlag = true; s.ephemeralBlocksWired = false }), wantConfirm: "get approvals will FAIL after minting"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			logs := captureSlog(t)
