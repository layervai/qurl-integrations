	...
	payload := map[string]any{
		...
		"blocks": []any{
			...
			sectionBlock(strings.Join([]string{
				...
				"`/qurl setalias $<name> <url-or-resource_id>` — configure a name in this channel",
				"`/qurl unsetalias $<name>` — remove a configured name in this channel",
				"`/qurl aliases` — list names configured in this channel",
				"`/qurl admin policies` — list channel/name policies",
				"`/qurl admin status` — workspace bot health and admin info",
				"`/qurl admin revoke $<name>` — revoke a previously minted link",
			}, "\n")),
			dividerBlock(),
