...
func (h *Handler) exposedChannelsForEdit(ctx context.Context, log *slog.Logger, meta *TunnelEditModalMetadata) []string {
	...
	if err != nil {
		log.Log(ctx, storeErrorLogLevel(ctx, err, slog.LevelWarn), "list edit: channel enumeration failed — pre-filling current channel only",
...
func (h *Handler) channelHasAliasForResource(ctx context.Context, log *slog.Logger, teamID, channelID, resourceID string) bool {
	...
	if err != nil {
		log.Log(ctx, storeErrorLogLevel(ctx, err, slog.LevelWarn), "tunnel edit: post-revoke alias re-check failed; reporting channel as revoked",
