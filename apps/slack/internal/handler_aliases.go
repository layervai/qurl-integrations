...
func (h *Handler) processAliases(ctx context.Context, log *slog.Logger, values url.Values) {
	...
	if err != nil {
		log.Log(ctx, storeErrorLogLevel(err, slog.LevelWarn), "aliases: rate-limit check failed", "error", err, "team_id", teamID, "user_id", userID)
	...
	if err != nil {
		...
		log.Log(ctx, storeErrorLogLevel(err, slog.LevelWarn), "aliases: GetChannelPolicy failed", "error", err, "team_id", teamID, "channel_id", channelID)
