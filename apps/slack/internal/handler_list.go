...
// listResourcesScanLimit is the page size for the single `/v1/resources` fetch
// backing `/qurl list`, `/qurl aliases`, and the URL resource-alias fallback
// in `/qurl get`. We over-fetch (the server's max) rather than page to a small
// display target: a 10-row page could surface zero channel-allowed resources
// after client-side filtering even when mintable rows exist. One generous page
// keeps the command single-request while making the client-side filtering
// reliable for normal workspaces. Bumping this affects all three surfaces.
//
// A server-side "mintable from Slack" filter (or allow-set-driven listing)
// would let this drop to a normal page size and make `page.HasMore` mean "more
// channel-listable resources" rather than "more resources of any type" — see
// the footer caveat in [Handler.processListResources] and the same caveat on
// the `/qurl aliases` triage warning in [Handler.processAliases].
...
func (h *Handler) processListResources(ctx context.Context, log *slog.Logger, values url.Values) {
	...
	// `/qurl list` shows only Slack-mintable resource types, then scopes to the
	// channel allow-set so only resources exposed to THIS channel render — every
	// member sees exactly the resources they could `/qurl get` here, no more.
	//
	// Known gap (#590): only the first listResourcesScanLimit (100) upstream
	// resources are scanned before filtering, so a resource exposed to this
	// channel but sorting past row 100 won't render here. For display this is
	// fail-safe under-disclosure, not a leak. For URL resource-alias minting,
	// however, the get fallback and duplicate-alias ambiguity detection share
	// the same scan; a duplicate alias outside the first page can be missed.
	// Channel aliases already bound in DDB are unaffected. Driving listing and
	// alias resolution by the allow-set IDs is tracked in #590.
	resources := filterResourcesAllowedInChannel(filterListableResources(page.Resources), allowed)
...
// resources they could mint here. The caller has already dropped non-mintable
// and revoked rows; this drops resources not exposed to the current channel.
