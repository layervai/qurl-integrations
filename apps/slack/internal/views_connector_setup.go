...
func S3WebsiteInstallModal(meta *TunnelInstallModalMetadata) ([]byte, error) {
	...
	payload := map[string]any{
		...
		blockKitFieldBlocks: []any{
			...
			inputBlock(s3WebsiteInstallBlockBucket, "S3 bucket", "Private bucket name, 3 to 63 characters. Dotted bucket names are not supported by this origin image.", false,
			...
			inputBlock(s3WebsiteInstallBlockPrefix, "S3 prefix", "Optional, up to 256 characters. Use this when the website files live under a folder such as website.", true,
				plainTextInput(s3WebsiteInstallActionPrefix, "website", "")),
			inputBlock(s3WebsiteInstallBlockIndex, "Directory index file", "Optional, up to 128 characters. Keep index.html unless your site uses default.html, home.html, or similar.", true,
