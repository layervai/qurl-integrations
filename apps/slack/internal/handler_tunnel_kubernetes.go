...
func renderKubernetesTunnelInstructions(args *tunnelInstallArgs, image string) (string, error) {
	...
	quotedKnockResourceID, err := yamlSingleQuoted(args.KnockResourceID)
	if err != nil {
		return "", err
	}
...
volumes:
  ...
  - name: qurl-proxy
    configMap:
      name: %s`, quotedImage, quotedSlug, quotedKnockResourceID, quotedAPIURL, quotedAgentPVC, quotedSecret, quotedConfigMap)
	...
	intro := strings.Join([]string{
		...
		"- The pod-level `fsGroup: 65532` lets the sidecar read the enrollment-token Secret and write the qURL agent-state PVC. If your app cannot accept that fsGroup, pre-provision qURL agent-state ownership separately before merging the fragment.",
		...
		"- Delete the enrollment-token Secret after the pod logs show the qURL Connector connected.",
	}, "\n")
