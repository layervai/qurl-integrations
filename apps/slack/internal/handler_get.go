...
func (h *Handler) getWork(ctx context.Context, log *slog.Logger, args *getWorkArgs) (string, error) {
	...
	if err != nil {
		log.Warn("get: rate-limit check failed", "error", err, "user_id", args.userID)
		return "", &userError{msg: rateLimitErrorMessage(err)}
	}
	if !ok {
		log.Info("get: rate-limit denied mint", "user_id", args.userID, "retry_after_ms", retry.Milliseconds())
