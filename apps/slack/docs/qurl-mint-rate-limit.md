...
Counter rows share the `channel_policies` team partition with channel policy rows. `ChannelsForResource` filters them out, but until TTL cleanup lands, the `/qurl list` Edit-modal prefill query and `/qurl revoke` channel-binding sweep still page over dormant counter rows. Storage remains bounded to one row per `(workspace, user)`, not one row per hour. Native TTL cleanup for inactive counter rows is tracked as the fast-follow layervai/qurl-integrations-infra#1225.
