...
| `QURL_CONNECTOR_HUB_HOST`, `QURL_CONNECTOR_HUB_PORT`, `QURL_CONNECTOR_HUB_SERVER_PUBLIC_KEY_B64` | Required together for sandbox/custom deployments | Explicit native Hub trust copied into every Docker, Compose, Kubernetes and ECS install, including S3 websites. Use a canonical LayerV hostname, port `443` and a padded base64 32-byte public key. Partial/invalid configuration stops startup. With all three absent, generated installs use the published CLI’s production Hub default; `QURL_ENDPOINT` alone does not select a sandbox Hub. |
