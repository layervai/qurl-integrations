...
| `QURL_CONNECTOR_HUB_HOST` | No | Connector Hub hostname rendered into `/qurl-admin protect-connector` install snippets. Set only while advertised builds are dark; omit once release images carry a pinned Hub. Must be set together with `QURL_CONNECTOR_HUB_PORT` and `QURL_CONNECTOR_HUB_SERVER_PUBLIC_KEY_B64` — startup validation fails a partial set. |
| `QURL_CONNECTOR_HUB_PORT` | No | Connector Hub port rendered alongside `QURL_CONNECTOR_HUB_HOST`; must be exactly `443`. Set only while advertised builds are dark; omit once release images carry a pinned Hub. Must be set together with the other two Hub trust envs — startup validation fails a partial set. |
| `QURL_CONNECTOR_HUB_SERVER_PUBLIC_KEY_B64` | No | Connector Hub server public key (X25519, base64-encoded, must decode to exactly 32 bytes) rendered alongside `QURL_CONNECTOR_HUB_HOST`. Set only while advertised builds are dark; omit once release images carry a pinned Hub. Must be set together with the other two Hub trust envs — startup validation fails a partial set. |
