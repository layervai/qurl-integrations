...
| `QURL_IMAGE_VERSION` | No | Optional. Human-readable qurl release behind the `QURL_IMAGE` digest (e.g. `v2.1.1`), shown beside the digest in rendered installs. Must look like a release version; a malformed value fails startup. |
...
| `QURL_CONNECTOR_HUB_PORT` | No | Canonical decimal port 1-65535 of the NHP Hub (e.g. `443`). |
| `QURL_CONNECTOR_HUB_SERVER_PUBLIC_KEY_B64` | No | Standard base64 of the 32-byte NHP Hub server public key the rendered installs pin. |
