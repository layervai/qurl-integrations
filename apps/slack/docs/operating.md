...
| `QURL_IMAGE_VERSION` | No | Human-readable `qurl` release behind the `QURL_IMAGE` digest (e.g. `v2.1.1`), shown beside the digest in rendered installs. An operator assertion: nothing cross-checks it against the digest. Ignored for the mutable dev/sandbox fallback image. Must look like a release version; a malformed value fails startup. |
...
| `QURL_CONNECTOR_HUB_HOST` | No | Optional, all-or-none with the two below. Bare DNS host name of the NHP Hub rendered into every Connector install (the released `qurl` CLI has no baked Hub pin). A partial triple or malformed value fails startup. |
| `QURL_CONNECTOR_HUB_PORT` | No | Canonical decimal port 1-65535 of the NHP Hub (e.g. `443`). All-or-none with the other two `QURL_CONNECTOR_HUB_*` variables. |
| `QURL_CONNECTOR_HUB_SERVER_PUBLIC_KEY_B64` | No | Canonical standard base64 of the 32-byte NHP Hub server public key the rendered installs pin. All-or-none with the other two `QURL_CONNECTOR_HUB_*` variables. |
