...
- `apps/teams/` — Node.js (TypeScript ESM, `src/*.ts`); Microsoft Teams bot, OAuth start/callback routes, `@microsoft/teams.apps` SDK adapter, and a production HTTP entrypoint (`bin: qurl-teams`)
...
The **Workflows** row is guarded separately because `check-extension-lockstep.sh` walks `apps/<name>/` trees keyed by a shared relative path, which those two files do not have. `TestExtensionWorkflowsStayInLockstep` in `internal/ciworkflows/workflow_test.go` covers them instead, masking each copy's app slug (`chrome-extension`), change output (`chrome_extension`), verifier env var (`CHROME_EXTENSION_CHANGED`) and browser name, then requiring a line-for-line match. It reads the first three out of that app's `requiredWorkflowSpecs` entry, so renaming one there cannot leave a stale duplicate widening what the test ignores. Two things differ from the script above. It also masks the indefinite article a browser name forces ("a Chrome" vs "an Edge"), including sentence-initial. And it masks each copy for *its own* slug and browser only — so unlike the script, an Edge workflow left saying "Chrome" is reported rather than accepted, at the cost that neither file can name the other (a cross-reference diverges under its own slug mask, which is why the pointer comment at the top of each file is sibling-agnostic). Living in `internal/ciworkflows/` also puts it on `Workflow Contract`, which is a required check; the Scripts workflow that runs the script is not, so drift it catches annotates a PR without blocking the merge.
