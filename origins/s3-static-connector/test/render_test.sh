...
grep -q 'proxy_cache_valid 200 1h30m;' "$TMP/composite-ttl/nginx.conf"
...
if render millisecond-ttl S3_BUCKET=example-bucket AWS_REGION=us-east-1 CACHE_DEFAULT_TTL=500ms 2>"$TMP/millisecond-ttl.err"; then
  echo "MISMATCH: millisecond CACHE_DEFAULT_TTL rendered successfully" >&2
  exit 1
fi
grep -q "millisecond TTLs are intentionally not supported" "$TMP/millisecond-ttl.err"
...
# The auth-failure access log is the only runtime signal that separates a
# rejected credential from a missing object once nginx has masked both to 404.
grep -q 'msg: s3_auth_failed' "$TMP/default/envoy.yaml"
